...
def execute_gcp_command(gcp_command):
    """Execute a Google Cloud command using gcloud CLI"""
    try:
        ...
        if gcloud_check.returncode != 0:
            # Try to install gcloud if not available
            install_cmd = """
            ...
            if install_result.returncode != 0:
                return {
                    "status": "ERROR",
                    "message": "gcloud CLI not available and installation failed",
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            env={**os.environ}  # Inherit all environment variables including GOOGLE_APPLICATION_CREDENTIALS
        )
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                return {
                    ...
                    "command": gcp_command,
                    "output": output_data,
                    "message": "Google Cloud command executed successfully"
                }
            ...
            return {
                ...
                "command": gcp_command,
                "error": result.stderr,
                "message": "Google Cloud command failed"
            }
    except Exception as e:
        return {
            "status": "ERROR",
            "message": f"Failed to execute GCP command: {str(e)}"
        }
...
def execute_agent_command(command):
    ...
    if command_lower.startswith("execute gcp "):
        gcp_command = command[12:]  # Remove "execute gcp " prefix
        return execute_gcp_command(gcp_command)
    
    # Configuration cleanup
    elif "analyze" in command_lower and "mcp-tools" in command_lower:
