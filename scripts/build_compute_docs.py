...
COMPUTE_DISC = os.path.join(BASE_DIR, "compute_discovery.json")
OUT_DIR = os.path.join(BASE_DIR, "compute")
os.makedirs(OUT_DIR, exist_ok=True)

with open(COMPUTE_DISC, "r", encoding="utf-8") as f:
    disc = json.load(f)

# ---- api_endpoints.json --------------------------------------------------
endpoints = []
base_url = disc.get("rootUrl", "").rstrip("/") + "/" + disc.get("servicePath", "").lstrip("/")
for op_id, m in disc.get("resources", {}).items():
    pass  # resources block not always structured per method
...
master["compute"] = {
    "base_url": base_url,
    "discovery_file": "compute_discovery.json",
    "endpoint_count": len(endpoints)
}
with open(master_index_path, "w", encoding="utf-8") as f:
    json.dump(master, f, indent=2)

print("Generated compute API documentation files.")
